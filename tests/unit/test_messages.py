...
class TestBGPOpenMessage:
    ...
    def test_open_message_parse(self):
        msg = BGPOpenMessage(version=4, my_as=65001, hold_time=180, bgp_id=0x0A000001)
        data = msg.serialize()
...
class TestBGPUpdateMessage:
    ...
    def test_update_message_with_nlri(self):
        msg = BGPUpdateMessage(
            withdrawn_routes=b"", path_attributes=b"", nlri=bytes([24, 192, 168, 1, 0])
        )
        data = msg.serialize()
        assert len(data) == 28
...
class TestBGPNotificationMessage:
    def test_notification_message(self):
        msg = BGPNotificationMessage(error_code=1, error_subcode=1, data=b"")
        data = msg.serialize()
        assert len(data) == 21
    ...
    def test_notification_message_parse(self):
        msg = BGPNotificationMessage(
            error_code=3, error_subcode=11, data=b"\x00\x01\x02"
        )
        data = msg.serialize()
...
class TestPathAttribute:
    ...
    def test_as_path_attribute(self):
        aspath_data = bytes([AS_PATH_SEGMENT_TYPES["AS_SEQUENCE"], 2]) + struct.pack(
            "!HH", 65001, 65002
        )
