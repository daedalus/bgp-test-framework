...
from .constants import (
    MARKER, MESSAGE_TYPES, MESSAGE_TYPE_NAMES, MESSAGE_MIN_LENGTHS,
    ORIGIN_TYPES, AS_PATH_SEGMENT_TYPES, PATH_ATTRIBUTE_FLAGS,
    PATH_ATTRIBUTE_TYPES, NOTIFICATION_ERROR_CODES,
...
class BGPOpenMessage:
    def __init__(self, version: int = 4, my_as: int = 0, hold_time: int = 180,
                 bgp_id: int = 0, optional_params: bytes = b''):
    ...
    def serialize(self) -> bytes:
        data = struct.pack('!BHHI', self.version, self.my_as, self.hold_time, self.bgp_id)
    ...
    def parse(cls, data: bytes) -> 'BGPOpenMessage':
        ...
        optional_params = data[offset+10:offset+10 + opt_param_len] if opt_param_len > 0 else b''
        
        return cls(version, my_as, hold_time, bgp_id, optional_params)


class BGPUpdateMessage:
    def __init__(self, withdrawn_routes: bytes = b'', 
    ...
    def serialize(self) -> bytes:
        ...
        data = withdrawn_len + self.withdrawn_routes + path_attr_len + self.path_attributes + self.nlri
    ...
    def parse(cls, data: bytes) -> 'BGPUpdateMessage':
        ...
        path_attr_len = struct.unpack('!H', data[2 + withdrawn_len:4 + withdrawn_len])[0]
...
class NLRIPrefix:
    ...
    def serialize(self) -> bytes:
        ...
        padded = prefix_bytes[:num_octets] + b'\x00' * (num_octets - len(prefix_bytes[:num_octets]))
    ...
    def parse(cls, data: bytes, offset: int = 0) -> Tuple['NLRIPrefix', int]:
        ...
        prefix = ipaddress.ip_network(f"{prefix_bytes[:num_octets].hex()}/{length}", strict=False)
        return cls(str(prefix.network_address), length), offset + 1 + num_octets


def build_open_message(my_as: int, hold_time: int = 180, bgp_id: int = 0,
    ...
    msg = BGPOpenMessage(version=4, my_as=my_as, hold_time=hold_time,
                         bgp_id=bgp_id, optional_params=optional_params)
    return msg.serialize()


def build_update_message(withdrawn_prefixes: Optional[List[Tuple[str, int]]] = None,
                         path_attributes: Optional[List[PathAttribute]] = None,
                         nlri_prefixes: Optional[List[Tuple[str, int]]] = None) -> bytes:
    import ipaddress
    ...
    if withdrawn_prefixes:
        for prefix, length in withdrawn_prefixes:
            ...
            withdrawn_data += bytes([length]) + ip.network_address.packed[:(length + 7) // 8]
...
def build_notification_message(error_code: int, error_subcode: int = 0,
                                data: bytes = b'') -> bytes:
...
def create_as_path_attribute(as_numbers: List[int], segment_type: int = 2) -> PathAttribute:
    if segment_type == AS_PATH_SEGMENT_TYPES['AS_SEQUENCE']:
        data = bytes([segment_type, len(as_numbers)]) + b''.join(struct.pack('!H', asn) for asn in as_numbers)
    else:
        data = bytes([segment_type, len(as_numbers)]) + b''.join(struct.pack('!H', asn) for asn in as_numbers)
...
def create_multi_exit_disc_attribute(med: int) -> PathAttribute:
    return PathAttribute(PATH_ATTRIBUTE_TYPES['MULTI_EXIT_DISC'], 0x80, struct.pack('!I', med))


def create_local_pref_attribute(local_pref: int) -> PathAttribute:
    return PathAttribute(PATH_ATTRIBUTE_TYPES['LOCAL_PREF'], 0x40, struct.pack('!I', local_pref))


def create_aggregator_attribute(as_number: int, router_id: str) -> PathAttribute:
    import ipaddress
    ip = ipaddress.ip_address(router_id)
    return PathAttribute(PATH_ATTRIBUTE_TYPES['AGGREGATOR'], 0xc0, struct.pack('!H', as_number) + ip.packed)
